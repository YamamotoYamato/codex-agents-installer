...
選択したディレクトリに既に `AGENTS.md` がある場合は、先に内容を表示します。最新版と一致する内容が既に含まれている場合は何も変更せずにスキップします。

`versions/` に置かれた過去版と一致する部分が既存ファイル内に見つかった場合は、現在入っているバージョンとこれから入れるバージョンを表示し、確認後にその一致部分だけを最新版に置換します。この確認は Enter のみで yes になります。どのバージョンにも一致しない場合は、最新版を末尾に追記します。
...
過去版の置換確認をテストする場合は、`CODEX_AGENTS_SAVE=yes` を指定できます。

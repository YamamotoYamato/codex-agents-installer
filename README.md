...
`versions/` に置かれた過去版と一致する部分が既存ファイル内に見つかった場合は、現在入っているバージョンとこれから入れるバージョンを表示し、確認後にその一致部分だけを最新版に置換します。この確認は Enter のみで yes になります。どのバージョンにも一致しない場合は、上書きまたは追記を選択できます。この選択は Enter のみで上書きになります。

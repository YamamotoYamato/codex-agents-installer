...
選択したディレクトリ直下の `config.toml` には、`versions/` の番号付き TOML にある設定を、`CODEX-AGENTS-INSTALLER` のコメント付き管理ブロックとして反映します。トップレベル設定だけでなく、`[notice]` などのテーブル配下の設定も更新できます。現在は `versions/2.toml` により `model_context_window = 136000` とフォント設定を更新します。新しいバージョンで項目を削除すると、管理ブロックからも削除されます。

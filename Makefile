...
test:
	tmp=$$(mktemp -d); mkdir "$$tmp/.codex" "$$tmp/.codex-hattori"; printf 'existing\n' > "$$tmp/.codex-hattori/AGENTS.md"; CODEX_AGENTS_HOME="$$tmp" CODEX_AGENTS_SELECT=2 ./install.sh; grep -q 'existing' "$$tmp/.codex-hattori/AGENTS.md"; grep -q '読みやすく' "$$tmp/.codex-hattori/AGENTS.md"; rm -rf "$$tmp"; echo "Unix test OK"
endif
